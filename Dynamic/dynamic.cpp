...
void resize(Table& table, int deltaSize);
string* resize(string* arr, int size, int deltaSize);
...
void createCinemaList(Table& table, string filmName, int* cinemaCounter)
{
	string* cinemaList = new string[*cinemaCounter];
	bool flag = true;
	...
	for (size_t i = 0; i < table.tableSize; i++)
	{
		if (table.infoList[i].movieName == filmName)
		{
			...
			if (flag)
			{
				cinemaList = resize(cinemaList, *cinemaCounter, 1);
...
void resize(Table& table, int deltaSize)
{
	Movie* tempInfoList = new Movie[table.tableSize + deltaSize];
	if (table.tableSize != 0)
	{
		for (size_t i = 0; i < table.tableSize && i < table.tableSize + deltaSize; i++)
		{
...
string* resize(string* arr, int size, int deltaSize)
{
	string* tempArray = new string[size + deltaSize];
	if (size != 0)
	{
		for (size_t i = 0; i < size; i++)
		{
